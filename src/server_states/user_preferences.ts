...
export const migrateUserPrefs = (defaultValue: any, storedState: any) => {
  ...
  if (defaultValue.version === 3 && storedState.version === 2) {
    result.typeface = storedState.font;
    delete result.font;
  }
...
const baseAtom = atomWithStorage<UserPreferences>(
  ...
  createLocalStorageConfig<UserPreferences>(migrateUserPrefs),
  { getOnInit: true }
);
